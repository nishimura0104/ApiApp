...
class ApiFragment: Fragment() {
        ...
        recyclerView.apply {
            ...
            addOnScrollListener(object: RecyclerView.OnScrollListener() { //Scrollを検知するListenerを実装する。これによって、RecyclerViewの下端に近づいた時に次のページを読み込んで、下に付け足す
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) { // dx はx軸方向の変化量(横) dy はy軸方向の変化量(縦) ここではRecyclerViewは縦方向なので、dyだけ考慮する
                    ...
                    if (!isLoading && lastVisibleItem >= totalCount -6) { // 読み込み中でない、かつ、現在のスクロール位置から下に５件見えてないアイテムがある
                        updateDate(true)
                    }
                }
    ...
    fun updateView() { // お気に入りが表示されたときの処理(Activityからコールされる)
        recyclerView.adapter?.notifyDataSetChanged() // RecyclerViewのAdapterに対して再描画のリクエストをする
    }

     fun updateDate(isAdd: Boolean = false, query: String = getString(R.string.api_keyword)) {
        if (isLoading) {
            return
        } else {
            isLoading = true
        ...
        val url = StringBuilder()
            .append(getString(R.string.base_url)) // https://webservice.recruit.co.jp/hptpepper/gourmet/v1/
            ...
            .append("&keyword=").append(query) // お店の検索ワード。ここでは例として｢ランチ｣を検索
            .append("&format=json") // ここで利用しているAPIは戻り値の形をxmlかjsonが選択することができる。Androidで扱う場合はxmlよりもjsonの方が扱いやすいので、jsonを選択
            .toString()
