        ...
        holder.apply {
            rootView.apply {
                setBackgroundColor(ContextCompat.getColor(context,
                        if (position % 2 == 0) android.R.color.white else android.R.color.darker_gray)) // 偶数番目と奇数番目で背景色を変更させる
                setOnClickListener {
                    val shop = Shop(
                        couponUrls = CouponUrls(sp = data.url, pc = data.url),
                        id = data.id,
